...
int main(void)
{
  ...
  for(int i = 0; i < sizeof(patterns)/sizeof(patterns[0]); i++){
    write_to_address(0x0000 + i, patterns[i]);
  }
...
//  uint8_t binary_Val = read_from_address(counter);


  if(patterns[counter] == read_from_address(counter) )
  {
    GPIOB-> ODR = read_from_address(counter);
    if(counter==5)
    {
      counter = 0;
    }
